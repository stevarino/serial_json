...
class TestParsing(unittest.TestCase):
    def check(self, value):
        self.assertEqual(next(sejson.loads(value))[1], json.loads(value))
    ...
    def test_arrays(self):
        result = list(sejson.loads('[1, 2, [3, 4], 5]'))
    ...
    def test_objects(self):
        result = list(sejson.loads('{"foo": 3, "bar": [{"baz": 4}]}'))
        self.assertEqual(result, [
            ('$.foo', 3.0),
            ('$.bar[0].baz', 4.0)
        ])
    ...
    def test_buffer_size(self):
        for i in range(2, 6):
            ...
            list(sejson.load(buf, buffer_size=i))
